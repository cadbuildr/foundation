...
class Circle(SketchElementMixin, ParameterFieldsMixin, BaseModel, Computable):
    """Generated from GraphQL object Circle."""
    ...
    def translate(self, dx: float, dy: float) -> Optional[Circle]:
        # Build local namespace with parameters for method function
        _locals = {
            'dx': dx,
            'dy': dy
        }
        return run_method(self, 'circle_translate', _locals)
    def rotate(self, angle: float, center: Optional[Point]=None) -> Optional[Circle]:
        # Build local namespace with parameters for method function
        _locals = {
            'angle': angle,
            'center': center
        }
        return run_method(self, 'circle_rotate', _locals)
