...
class Polygon(SketchElementMixin, BaseModel, Computable):
    """Generated from GraphQL object Polygon."""
    ...
    def translate(self, dx: float, dy: float) -> Optional[Polygon]:
        # Build local namespace with parameters for method function
        _locals = {
            'dx': dx,
            'dy': dy
        }
        return run_method(self, 'polygon_translate', _locals)
    def rotate(self, angle: float, center: Optional[Point]=None) -> Optional[Polygon]:
        # Build local namespace with parameters for method function
        _locals = {
            'angle': angle,
            'center': center
        }
        return run_method(self, 'polygon_rotate', _locals)


    lines: List[Line] = Field(...)
    sketch: Optional[Sketch] = Field(default=None, json_schema_extra={'compute': {'expr': 'lines[0].sketch if lines else None'}})
