...
class Ellipse(SketchElementMixin, ParameterFieldsMixin, BaseModel, Computable):
    """Generated from GraphQL object Ellipse."""
    ...
    def translate(self, dx: float, dy: float) -> Optional[Ellipse]:
        # Build local namespace with parameters for method function
        _locals = {
            'dx': dx,
            'dy': dy
        }
        return run_method(self, 'ellipse_translate', _locals)
    def rotate(self, angle: float, center: Optional[Point]=None) -> Optional[Ellipse]:
        # Build local namespace with parameters for method function
        _locals = {
            'angle': angle,
            'center': center
        }
        return run_method(self, 'ellipse_rotate', _locals)
