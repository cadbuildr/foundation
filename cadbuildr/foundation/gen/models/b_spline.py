...
class BSpline(ParameterFieldsMixin, BaseModel, Computable):
    """Generated from GraphQL object BSpline."""
    ...
    def translate(self, dx: float, dy: float) -> Optional[BSpline]:
    ...
    def rotate(self, angle: float, center: Optional[Point]=None) -> Optional[BSpline]:
        # Build local namespace with parameters for method function
        _locals = {
            'angle': angle,
            'center': center
    ...
    degree: IntParameter = Field(default_factory=lambda: _eval_expr({}, 'IntParameter(value=3)'), json_schema_extra={'default': {'expr': 'IntParameter(value=3)'}})
