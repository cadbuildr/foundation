...
def main(argv: List[str] | None = None) -> int:
    ...
    try:
        from cadbuildr.foundation import utils_webrtc as webrtc_module  # type: ignore
    ...
    if not args.no_feedback and utils_webrtc and hasattr(utils_webrtc, "collect_and_display_feedback"):
        from cadbuildr.foundation.utils_webrtc import collect_and_display_feedback
...
def get_screenshot_main(argv: List[str] | None = None) -> int:
    ...
    parser.add_argument(
        '--broker-url',
        help='Optional broker base URL (default: http://localhost:5000)',
    )
    parser.add_argument(
        '-o', '--output',
    ...
    try:
        ...
        import requests
        broker_url = args.broker_url or "http://localhost:5000"
        
        # Request screenshot from viewer - broker will wait synchronously for response
        response = requests.post(
            f"{broker_url}/screenshot/request",
            json={},
            timeout=15.0
        )
