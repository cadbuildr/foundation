...
BROKER_URL = "http://localhost:5050/send"
_RECORDED_REQUEST_IDS: list[str] = []


def create_dag_message(
    dag: Dict[str, Any], request_id: Optional[str] = None
) -> Dict[str, Any]:
    """Create a typed DAG message for the broker."""
    return {"type": "@buildr/dag", "dag": dag, "request_id": request_id}
...
def set_broker_url(url: str):
    ...
    Args:
        url: Full URL to the broker's HTTP endpoint (e.g., 'http://localhost:5050/send')
    """
    global BROKER_URL
    BROKER_URL = url
...
def set_port(port: int):
    """
    Set the broker's HTTP port.

    Args:
        port: HTTP port of the broker (default: 5050)
    """
    global BROKER_URL
...
def show_ext(dag: Any, request_id: Optional[str] = None) -> Optional[str]:
    ...
    try:
        ...
        # Send DAG to broker
        response = requests.post(BROKER_URL, json=message, timeout=2.0)
...
def get_build_status(
    request_id: str, broker_url: Optional[str] = None
    ...
    try:
        ...
        response = requests.get(f"{broker_url}/status/{request_id}", timeout=2.0)
...
def get_screenshot(request_id: str, broker_url: Optional[str] = None) -> Optional[str]:
    """
    ...
    try:
        ...
        response = requests.get(f"{broker_url}/screenshot/{request_id}", timeout=2.0)
