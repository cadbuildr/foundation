...
def solve_at3_bt2_ct_d(x0: float, x1: float, xp0: float, xp1: float) -> tuple[float, float, float, float]:
...
class TwoPointsSpline:
    ...
    def get_xy_coeffs(self, smooth_factor: float | None = None) -> tuple[tuple[float, float, float, float], tuple[float, float, float, float]]:
        p1 = self.p1.p
        p2 = self.p2.p
